...
\lhead{Neural Information Processing - 4\_probabilistic\_senses - Conor}
\rhead{\thepage}
\cfoot{}
\begin{document}

\section*{Sensory processing and probabalist codes.}
...
\end{equation}
Multiplying all this out, or using the formula for multiplying
Gaussians, this means that $p(x|v,h)$ is given by
$\mathcal{N}(\bar{x},\sigma)$ with
\begin{equation}
\frac{1}{\sigma^2}=\frac{1}{\sigma_v^2}+\frac{1}{\sigma_h^2}
\end{equation}
and 
\begin{equation}
\bar{x}=\frac{\sigma_v^2}{\sigma^2}v+\frac{\sigma_h^2}{\sigma^2}h
\end{equation}
...
subjects are weighting the two pieces of evidence; for details see the
paper. They then compare the measured weightings to those estimated by
the optimal Bayesian model. The result is shown in
Fig.~\ref{fig_weights} and demonstrate Bayesian optimal perception in
this aspect of human sensory fusion.
...
\caption{Results of the Ernst and Banks experiment. As the weights are
  ...
  these are the red features. The match is very good.[Pictures from
    Rosalyn Moran who took it from the paper]\label{fig_ernstbanks}}
\end{figure}
