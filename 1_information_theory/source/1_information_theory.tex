...
&=&-\sum_{x,y} p_{X,Y}(x,y)\log_2{p_{X|Y}(x|y)p_Y(y)}\cr
&=&-\sum_{x,y} p_{X|Y}(x|y)p_Y(y)\log_2{p_{X|Y}(x|y)}-\sum_y p_{Y}(y)\log_2{p_Y(y)}
\end{eqnarray}
or
\begin{equation}
H(X,Y)=H(X)+H(Y|X)
...
determined by $X$ then $H(X|Y)=0$ and $H(X,Y)=H(X)$ and $I(X,Y)=H(Y)$.
...
where, roughly speaking $Z$ only learns about $X$ though $Z$; it isn't
that $X$ and $Z$ are independent, just that if you know the value of
$Y$ knowing the value of $X$ wouldn't tell you anything. A game of
...
and $Y$ are independent. The data processing inequality states that in this situation
\begin{equation}
I(X,Y)\ge I(X,Z)
\end{equation}
