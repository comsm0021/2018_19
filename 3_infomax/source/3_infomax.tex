...
only approach to the problem and a different algorithm, fastICA, is
the most popular \cite{Hyvarinen1999} algorithm.
...
mixing of auditory signals in a room will only have these to problems
...
$p_{S_1,S_2}=p_{S_1}p_{S_2}$. Finally, we assume they are mixed
linearly so that we record $r_1(t)$ and $r_2(t)$ with \begin{equation}
{\bf r}(t)=M{\bf s}(t) \end{equation} where $M$ is a constant mixing
matrix. Now, we want to find an unmixing matrix $W$ so that knowing
...
the formula above, you can see, (C\&T Theorem 8.6.4), that
\begin{equation} 
...
\begin{equation}
\langle g(X)\rangle =\int dx p_X(x)g(x)
\end{equation}
